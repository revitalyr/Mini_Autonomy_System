...
auto AsyncProcessingPipeline::processingLoop() noexcept -> void {
    while (m_pimpl->is_running.load()) {
        ...
        auto tracks = m_pimpl->tracker->update(detections);
        
        auto end_time = Clock::now();
        m_pimpl->metrics->record_frame_latency(static_cast<double>(to_milliseconds(end_time - start_time).count()));
        
        m_pimpl->result_queue->push({std::move(*img_opt), std::move(tracks)});
    }
}
