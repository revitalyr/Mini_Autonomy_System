/**
 * @file rosbag_viz_demo.cpp
 * @brief ROSBAG visualization demo with OpenCV
 *
 * This demo visualizes ROSBAG data with OpenCV windows showing
 * images and IMU data overlays.
 *
...
Expected<void, PerceptionError> demo_rosbag_visualization(const std::string& bag_path) {
    ...
    cv::namedWindow("ROSBAG Visualization", cv::WINDOW_AUTOSIZE);
    ...
    for (auto frame_result : generator) {
        ...
        if (!frame.imu_samples.empty()) {
            ...
            // Create overlay text
            std::string info_text = cv::format(
                "Frame: %d | Time: %.3f s\n"
                "Accel: (%.3f, %.3f, %.3f) m/s²\n"
                "Gyro: (%.3f, %.3f, %.3f) rad/s\n"
                "IMU samples: %zu",
        ...
        // Display image
        cv::imshow("ROSBAG Visualization", cv_img);
        ...
        // Wait for key press (30ms delay for ~30fps playback)
...
auto main(int argc, char* argv[]) -> int {
    try {
        ...
        if (!std::filesystem::exists(bag_path)) {
            ...
            std::cout << "Usage: rosbag_viz_demo [path_to_bag_file]\n";
            return 1;
        }
        ...
        if (auto result = perception::demo_rosbag_visualization(bag_path); !result) {
